...
# Embed endpoint for "I want" prompts
...
async def create_embed(request: EmbedRequest):
    """Create an embed URL for Google Maps or YouTube based on query"""
    try:
        embed_tool = get_embed_tool()
        
        if request.embed_type == "google_maps":
            if not embed_tool.maps_enabled:
            ...
            if not embed_url:
                raise HTTPException(status_code=500, detail="Failed to create Google Maps embed")
            ...
            if not video_id:
                raise HTTPException(status_code=404, detail="No YouTube video found for query")
            ...
            raise HTTPException(status_code=400, detail=f"Invalid embed_type: {request.embed_type}")
